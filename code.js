...
async function setImageFromName(name) {
...
async function setImageFromFormula(formula) {
...
async function setImage(formula, name) {
  if (name !== '') {
...
function processFormula(formula, subprocess) {
  ...
  if (subprocess !== false) {
    ...
    if (namesFinal === namesCombined.join(', ')) {
      let old = namesFinal;
      namesFinal = namesFinal.replace(/, Oxygen, Hydrogen$/, ' Hydroxide');
      namesFinal = namesFinal.replace(/, Hydrogen, Carbon, Oxygen \(x3\)$/, ' Bicarbonate');
      namesFinal = namesFinal.replace(/, Carbon, Oxygen \(x3\)$/, ' Carbonate');

      if (namesFinal.replace(/[^,]/g, "").length === 1) {
        let newNames = namesCombined.slice(0);

        for (let i = 0; i < newNames.length; i++) {
          newNames[i] = newNames[i].replace(/ygen\b/g, 'ide');
          newNames[i] = newNames[i].replace(/ine\b/g, 'ide');

          if (i !== 0) {
            newNames[i] = newNames[i].replace(/^([A-Za-z]{1,})$/, function (_, el) {
              if (elements[0].desc.indexOf('metal') === -1) {
...
function popupify(el) {
  el.onmouseleave = function() {
    document.getElementById('popup').className = '';

    let result = processFormula(f.value);

    let body = result[0];

    let revCompound = getCompoundByName(f.value);
    ...
    if (body !== undefined && body !== '' && el.parentNode.innerText.indexOf('/') === -1) {
      ...
      if (dashSplit.length === 2 && dashSplit[0].indexOf('*') === -1) {
        ...
        name = revCompound === undefined ? name : titleCase(f.value);
  ...
  el.onmouseenter = function() {
    ...
    let num = 1;
    let name = unsubscriptise(el.innerText).replace(',', '').replace(' /', '').replace('*', '').replace(/[0-9]/g, (_) => { num = _; return ''; }).trim();
    ...
    if (element !== undefined) {
      ...
      // setImage(getSymbolFromElement(element), name);
    }
...
function interpretInput(again = false) {
  ...
  if (body !== undefined && body !== '') {
    ...
    if (dashSplit.length === 2 && dashSplit[0].indexOf('*') === -1) {
      ...
      // setImage(unsubscriptise(value), dashSplit[0].replace('*', '').split(' / ')[0]);
    } else {
      // setImage(unsubscriptise(value), '');
    }

    //setImageFromName(dashSplit[0].replace('*', '').split(' / ')[0]);
...
function highlightFormula(location, length) {
  let foo = document.createElement('div');
  foo.style.position = 'absolute';
  foo.style.width = `${length * 40}px`;
  foo.style.height = '15%';
  foo.style.left = `${f.getBoundingClientRect().x + (f.getBoundingClientRect().width / 2) - (foo.getBoundingClientRect().width / 2) + (location * 40)}px`;
  foo.style.top = `${f.offsetTop}px`;
  foo.style.backgroundColor = 'red';
  foo.style.opacity = '0.5';
  foo.style.pointerEvents = 'none';
